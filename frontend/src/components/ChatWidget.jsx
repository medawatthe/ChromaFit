          ...
          <div className="flex-1 space-y-3 overflow-y-auto p-3">
            ...
            {loaded && history.length === 0 && (
              <p className="text-xs text-gray-400">
                Ask me "What should I wear tomorrow?" or anything about your wardrobe.
              </p>
            )}
            {history.map((entry) => (
              <div key={entry.id} className="space-y-1">
                <div className="flex justify-end">
                  <div className="max-w-[85%] rounded-2xl rounded-br-sm bg-brand-600 px-3 py-1.5 text-xs text-white">
                    {entry.message}
                  </div>
                </div>
